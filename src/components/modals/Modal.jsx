      ...
      <div className="flex flex-col items-center justify-center gap-5 text-white bg-gray-600 rounded-xl px-10 pt-5 pb-10 mx-4">
        ...
        >
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 mt-2 bg-white rounded-md px-3 py-2 h-96 overflow-y-scroll">
              {searchData?.map((data, index) => (
                <div
                  onClick={() => toggleMember(data.username)}
                  key={index}
                  className="user-row"
                >
                  <span>{data.username}</span>
                  {membersList.includes(data.username) ? (
          ...
          <button
            type="submit"
            className="mt-4 w-full flex items-center justify-center gap-2 px-5 py-3 font-medium rounded-md"
          >
