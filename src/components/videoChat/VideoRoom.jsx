...
const TOKEN =
  "007eJxTYHArC1tTEXHpReXNM2ekmVg3yDLXu1pM2ui0JC2tsPTlkkMKDBZplkmWponGSaaWSSaWaQZAwsw0xdzMNNXSwDAxxVzcdFV6QyAjA9fKYlZGBggE8dkY8osS89JTGRgAT2oe6A==";
...
function VideoRoom() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4">
      <div
        className={`grid gap-2 p-2 w-full ${getGridLayout(users.length)}`}
      ...
      >
        {users.map((user) => (
          <VideoPlayer
            ...
            user={user}
            style={{
              width: getMaxWidth(users.length),
              height: getVideoHeight(users.length),
            }}
          />
        ))}
      </div>
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex gap-4">
        <button
          className={`rounded-full border-none w-14 h-14 flex justify-center items-center transition-colors ${
            mic ? "bg-white" : "bg-red-600"
          }`}
          onClick={toggleMic}
        >
          {mic ? (
